...
void main() {
  ...
  test(
    ...
    () async {
      final source = _MemoryRepository(
        periods: [
          PeriodEpisode(id: 'synthetic-source', start: DateTime(2026, 8, 1)),
        ],
      );
      ...
      await expectLater(
        CycleVaultService(repository: target)
            .restoreBytes(truncated, passphrase),
        throwsA(anything),
      );
  ...
  test('corrupt CycleVault encoding is rejected while existing information survives', () async {
    final target = _MemoryRepository(
      periods: [
        PeriodEpisode(id: 'existing-information', start: DateTime(2026, 7, 1)),
      ],
    );
  ...
  test(
    ...
    () async {
      ...
      await expectLater(
        CycleVaultService(repository: target)
            .restoreBytes(futureSchema, passphrase),
