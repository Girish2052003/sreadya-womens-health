...
void main() {
  test('independent CycleVault v1 vector matches Dart Argon2id parameters', () async {
  ...
  test('independent CycleVault v1 vector decrypts through production Dart restore', () async {
    final vector = await _loadVector();
    final target = _MemoryRepository(
      periods: [PeriodEpisode(id: 'live-before-vector', start: DateTime.utc(2026, 7, 1))],
    );
    ...
    expect(target.periods.single.start.toUtc().toIso8601String(), '2026-08-01T00:00:00.000Z');
    expect(target.periods.single.end?.toUtc().toIso8601String(), '2026-08-04T00:00:00.000Z');
  ...
  test('wrong passphrase and ciphertext tamper preserve live Dart vault', () async {
    final vector = await _loadVector();

    final wrongPassTarget = _MemoryRepository(
      periods: [PeriodEpisode(id: 'live-wrong-pass', start: DateTime.utc(2026, 7, 1))],
    );
    await expectLater(
    ...
    final outer = jsonDecode(vector['containerJson'] as String) as Map<String, dynamic>;
    ...
    final tamperTarget = _MemoryRepository(
      periods: [PeriodEpisode(id: 'live-tamper', start: DateTime.utc(2026, 7, 2))],
    );
    await expectLater(
  ...
  test('unsupported CycleVault schema is rejected before replacing live Dart vault', () async {
    ...
    final target = _MemoryRepository(
      periods: [PeriodEpisode(id: 'live-schema', start: DateTime.utc(2026, 7, 3))],
    );
...
Future<Map<String, dynamic>> _loadVector() async {
  ...
  if (decoded is! Map<String, dynamic>) {
    throw const FormatException('CycleVault interoperability vector must be an object.');
  }
  return decoded;
}
