...
void main() {
  test('migrates an old schema through every required version', () {
    ...
    final coordinator = MigrationCoordinator(
      targetVersion: 3,
      steps: {
        1: (database) => database.execute('CREATE TABLE one(id INTEGER PRIMARY KEY);'),
        2: (database) => database.execute('CREATE TABLE two(id INTEGER PRIMARY KEY);'),
      },
    );
    ...
    expect(db.select("SELECT name FROM sqlite_master WHERE name='one';"), isNotEmpty);
    expect(db.select("SELECT name FROM sqlite_master WHERE name='two';"), isNotEmpty);
  });
  ...
  test('rolls back schema and data when a migration fails', () {
    ...
    final coordinator = MigrationCoordinator(
      targetVersion: 3,
      steps: {
        1: (database) => database.execute('CREATE TABLE should_rollback(id INTEGER PRIMARY KEY);'),
    ...
    expect(db.select("SELECT name FROM sqlite_master WHERE name='should_rollback';"), isEmpty);
  });
}
