...
class _ReportsScreenState extends ConsumerState<ReportsScreen> {
  late Set<ReportCategory> _selected = {...ReportSelection.safeDefault().categories};
  bool _busy = false;
  ...
  Future<void> _generate(bool pdf) async {
    ...
    try {
      ...
      final path = pdf
          ? await service.generatePdf(from: from, to: to, selection: ReportSelection(categories: _selected))
          : await service.generateCsv(from: from, to: to, selection: ReportSelection(categories: _selected));
      await SharePlus.instance.share(ShareParams(files: [XFile(path)], text: 'Sreva health report — shared intentionally by the user.'));
    } catch (error) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Report failed: $error')));
    } finally {
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          Card(
            child: Column(
              children: ReportCategory.values.map((category) => CheckboxListTile(
          ...
          FilledButton.icon(onPressed: _busy ? null : () => _generate(true), icon: const Icon(Icons.picture_as_pdf_outlined), label: const Text('Generate & share PDF locally')),
          const SizedBox(height: 8),
          OutlinedButton.icon(onPressed: _busy ? null : () => _generate(false), icon: const Icon(Icons.table_view_outlined), label: const Text('Generate & share CSV locally')),
          const SizedBox(height: 16),
  ...
  bool _isHighlyPrivate(ReportCategory value) => value == ReportCategory.privateNotes || value == ReportCategory.sexualActivity;
