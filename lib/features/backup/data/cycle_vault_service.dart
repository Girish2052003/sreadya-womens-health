...
class CycleVaultService {
  CycleVaultService({required HealthRepository repository}) : _repository = repository;

  static const String format = 'SREVA-CYCLEVAULT';
  static const int formatVersion = 1;
  final HealthRepository _repository;
  final AesGcm _cipher = AesGcm.with256bits();
  final Argon2id _kdf = Argon2id(memory: 19 * 1024, parallelism: 1, iterations: 2, hashLength: 32);

  Future<String> exportToFile(String passphrase) async {
    if (passphrase.length < 12) {
      throw ArgumentError('CycleVault recovery passphrase must be at least 12 characters.');
    }
    ...
    final key = await _kdf.deriveKeyFromPassword(password: passphrase, nonce: salt);
    final manifest = {
      'format': format,
    ...
    final name = 'sreva-${DateTime.now().toUtc().toIso8601String().replaceAll(':', '-')}.cyclevault';
  ...
  Future<CycleVaultRestoreResult> restoreFromFile(
    ...
    final outer = jsonDecode(await File(path).readAsString()) as Map<String, dynamic>;
    ...
    final periods = (payload['periods'] as List<dynamic>? ?? const [])
        .map((value) => _periodFromJson(Map<String, dynamic>.from(value as Map)))
        .toList(growable: false);
    final observations = (payload['observations'] as List<dynamic>? ?? const [])
        .map((value) => _observationFromJson(Map<String, dynamic>.from(value as Map)))
        .toList(growable: false);
  ...
  bool _validateStaged(_StagedRestore staged) {
    ...
    final ordered = [...staged.periods]..sort((a, b) => a.start.compareTo(b.start));
    for (var i = 0; i < ordered.length; i++) {
      final current = ordered[i];
      if (!periodIds.add(current.id)) return false;
      if (current.end != null && current.end!.isBefore(current.start)) return false;
      if (i > 0) {
  ...
  HealthObservation _observationFromJson(Map<String, dynamic> json) => HealthObservation(
        ...
        severity: json['severity'] == null ? null : ObservationSeverity.values.byName(json['severity'] as String),
