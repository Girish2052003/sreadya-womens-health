...
class _AssistantScreenState extends ConsumerState<AssistantScreen> {
  ...
  Future<void> _parse() async {
    final command = LocalIntentParser().parse(_controller.text, now: DateTime.now());
    String? answer;
    if (command.intent == LocalIntent.nextPeriodQuery) {
      ...
      answer = recent.isEmpty
          ...
          : recent.map((period) => DateFormat.yMMMd().format(period.start)).join(' · ');
    }
    if (!mounted) return;
    setState(() {
      _command = command;
    ...
    if (!await voice.supportsOfflineRecognition()) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Offline voice recognition is unavailable on this device/language. Nothing was uploaded.')),
        );
      }
      return;
    }
  ...
  Future<void> _confirm() async {
    ...
    if (command.intent == LocalIntent.periodStarted) {
      await ref.read(healthActionsProvider).startPeriod(command.date ?? DateTime.now());
    } else if (command.intent == LocalIntent.periodEnded) {
      await ref.read(healthActionsProvider).endLatestPeriod(command.date ?? DateTime.now());
    } else if (command.intent == LocalIntent.logSymptom || command.intent == LocalIntent.logFlow) {
      ...
      if (command.reminderHour == null || command.reminderMinute == null) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Please include a time, for example “Remind me about my medicine at 8 PM”.')),
          );
        }
        return;
      }
      ...
      await ref.read(reminderSchedulerProvider).schedule(plan, ReminderPlanner().notificationBody(plan));
    } else if (command.intent == LocalIntent.unknown) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('I could not safely turn that into a local health action. Nothing was saved.')),
        );
      }
      return;
    }
        ...
        children: [
          const Card(
            child: Padding(
              ...
              child: Text('Try: “My period started yesterday”, “I have bad cramps”, “Yesterday was heavy”, “When is my next period?”, or “Remind me about my medicine at 8 PM”. Parsing happens locally.'),
            ),
          ),
          ...
          FilledButton(onPressed: _controller.text.trim().isEmpty ? null : _parse, child: const Text('Understand locally')),
          if (_command != null) ...[
            ...
            Card(
              child: Padding(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                  Text('Sreva understood', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w700)),
                  const SizedBox(height: 8),
                  Text('Intent: ${_command!.intent.name}'),
                  if (_command!.date != null) Text('Date: ${DateFormat.yMMMd().format(_command!.date!)}'),
                  if (_command!.label != null) Text('Detail: ${_command!.label}'),
                  if (_command!.reminderHour != null)
                    Text('Reminder time: ${_command!.reminderHour!.toString().padLeft(2, '0')}:${(_command!.reminderMinute ?? 0).toString().padLeft(2, '0')}'),
                  if (_answer != null) ...[
                    const SizedBox(height: 12),
                  ...
                  if (_command!.requiresConfirmation)
                    FilledButton.icon(onPressed: _confirm, icon: const Icon(Icons.check), label: const Text('Confirm')),
                ]),
              ),
            ),
          ],
