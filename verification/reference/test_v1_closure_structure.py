...
def test_worldwide_formatters_are_wired():
    src = read("lib/core/settings/user_formatters.dart")
    assert "formatClock" in src
    assert "temperatureUnit" in src
    assert "weightUnit" in src
    assert "waterUnit" in src
