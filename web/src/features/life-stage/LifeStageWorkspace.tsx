...
export function LifeStageWorkspace(){
 ...
 const save=async()=>{const repository=repositoryRef.current;if(!repository)return;setSaving(true);setError('');try{await repository.save(mode);setStatus(t('core.saved'));}catch(cause){setError(cause instanceof Error?cause.message:t('lifeStage.saveError'));}finally{setSaving(false);}};
