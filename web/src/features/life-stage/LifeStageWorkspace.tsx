...
export function LifeStageWorkspace(){
 ...
 const save=async()=>{const repository=repositoryRef.current;if(!repository)return;setSaving(true);setError('');try{await repository.save(mode);setStatus(t('core.saved'));}catch(cause){setError(t('lifeStage.saveError'));}finally{setSaving(false);}};
