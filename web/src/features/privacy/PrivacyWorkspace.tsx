...
export function PrivacyWorkspace(){
 ...
 {ready&&privacy?<div className="workspace-grid">
  <Card eyebrow={t('privacy.centerEyebrow')} title={t('privacy.centerTitle')}><dl className="privacy-status-list">{(Object.keys(statusKeys) as Array<keyof WebPrivacyStatus>).map(key=><div key={key} className="privacy-status-row"><dt>{t(statusKeys[key])}</dt><dd>{t(privacy[key])}</dd></div>)}</dl></Card>
