...
import { REPORT_CATEGORIES,SAFE_REPORT_CATEGORIES,buildCsvReport,buildPdfReport,buildReportPreview,type ReportCategory,type ReportCopy } from './report-builder';
...
export function ReportsWorkspace(){
 ...
 const reportCopy:ReportCopy={title:t('reports.export.title'),disclaimer:t('reports.export.disclaimer'),periodsHeading:t('reports.category.periods'),ongoing:t('reports.export.ongoing'),observationsHeading:t('reports.export.observations'),pdfSubject:t('reports.export.pdfSubject'),pdfProducer:t('reports.export.pdfProducer'),kindLabel:(kind)=>t(`observation.kind.${kind}`),severityLabel:(severity)=>t(`observation.severity.${severity}`),flowLabel:(flow)=>t(`observation.flow.${flow}`)};
 ...
 const prepare=()=>{setError('');try{setPreview(buildReportPreview({periods,observations,selection},reportCopy));}catch{setPreview(null);setError(t('reports.previewError'));}};
 ...
 const downloadPdf=async()=>{setError('');try{downloadBlob(pdfBlob(await buildPdfReport({periods,observations,selection},reportCopy)),'sreadya-health-report.pdf');}catch{setError(t('reports.pdfError'));}};
