// Product-depth audit anchor: global I18nProvider owns locale activation; Settings deliberately has no second language selector.
'use client';
...
export function SettingsWorkspace(){
 const {t,locale,availableLocales}=useI18n();const[preferences,setPreferences]=useState<Preferences>({...defaults,locale}),[saved,setSaved]=useState(false);
 ...
 const language=currentLanguageChoice(locale,availableLocales);
 ...
 <div className="settings-language-summary"><span>{t('settings.language')}</span><strong dir={language.direction}>{language.nativeName}</strong><small>{t('settings.languageHomeOnly')}</small></div>
