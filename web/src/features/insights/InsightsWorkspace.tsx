...
export function InsightsWorkspace(){
 const {t,plural}=useI18n(); const [snapshot,setSnapshot]=useState<InsightSnapshot|null>(null),[ready,setReady]=useState(false),[status,setStatus]=useState(()=>t('core.vault.opening')),[error,setError]=useState('');
 ...
 const hasData=snapshot!==null&&(snapshot.provenance.dateRange!==null||snapshot.timingInsights.length>0||snapshot.pmsPatterns.length>0||snapshot.topFlowPattern!==null);
 const sourceName=(source:RecordSource)=>t(`source.${source}`);
 const observationName=(kind:ObservationKind)=>t(`observation.kind.${kind}`,{},kind);
 ...
 {ready&&!error&&snapshot?<div className="workspace-grid">
  ...
  <Card eyebrow={t('insights.pmsEyebrow')} title={t('insights.pmsTitle')}>{snapshot.pmsPatterns.length?<ul>{snapshot.pmsPatterns.map((pattern)=><li key={pattern.kind}>{plural('insights.pmsPattern',pattern.count,{symptom:observationName(pattern.kind),count:pattern.count})}</li>)}</ul>:<p>{t('insights.noPms')}</p>}</Card>
  ...
  <Card eyebrow={t('insights.patternsEyebrow')} title={t('insights.patternsTitle')}>{snapshot.timingInsights.length||snapshot.topFlowPattern?<ul>{snapshot.timingInsights.map((pattern)=><li key={pattern.kind}>{t('insights.timingPattern',{symptom:observationName(pattern.kind),day:pattern.cycleDay})}</li>)}{snapshot.topFlowPattern?<li>{plural('insights.flowPattern',snapshot.topFlowPattern.count,{flow:t(`observation.flow.${snapshot.topFlowPattern.flow}`,{},snapshot.topFlowPattern.flow),count:snapshot.topFlowPattern.count})}</li>:null}</ul>:<p>{t('insights.noPattern')}</p>}<p className="workspace-note">{t('insights.patternBoundary')}</p></Card>
