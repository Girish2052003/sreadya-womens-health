...
export function DevicesWorkspace(){
 ...
 const refresh=async()=>{if(!endpoint||!accountId)return;setError('');try{const response=await fetch(endpoint+'/v1/accounts/'+encodeURIComponent(accountId)+'/devices',{credentials:'include',cache:'no-store'});if(!response.ok)throw new Error(t('devices.error.request'));const value=await response.json() as Device[];setDevices(Array.isArray(value)?value:[]);}catch(cause){setError(t('devices.error.list'));}};
 const inspectQr=()=>{setError('');try{const parsed=parseTrustedDeviceQr(qr.trim());parsed.transferSecret.fill(0);setQrStatus(t('devices.validQr',{id:parsed.enrollmentId}));}catch(cause){setQrStatus('');setError(t('devices.invalidQr'));}};
