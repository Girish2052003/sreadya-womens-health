...
export function RecoveryWorkspace(){
 ...
 const verify=async()=>{setStatus('');setError('');let recoverySecret:Uint8Array<ArrayBuffer>|null=null,root:Uint8Array<ArrayBuffer>|null=null;try{recoverySecret=decodeHex(secret,t('recovery.keyError'));root=await unwrapRecoveryEnvelope(recoverySecret,JSON.parse(envelope) as RecoveryEnvelope);setStatus(t('recovery.success'));}catch(cause){setError(cause instanceof Error?cause.message:t('recovery.error'));}finally{recoverySecret?.fill(0);root?.fill(0);}};
