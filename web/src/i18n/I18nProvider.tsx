...
import { hasCompleteTranslation } from './translation-manifest';
...
type RuntimeManifest = {
  artifacts: Record<string, { path: string; coverage?: string; messageCount?: number; sourceMessageCount?: number }>;
};
...
async function loadManifest(): Promise<RuntimeManifest | null> {
  ...
  manifestPromise = fetch(`${basePath}/i18n/manifest.json`, { cache: 'force-cache' })
    .then(async (response) => {
      ...
      return { artifacts: artifacts as RuntimeManifest['artifacts'] };
    })
    .catch(() => null);
  return manifestPromise;
}

async function loadCompleteBundle(locale: string): Promise<Record<string, string> | null> {
  const normalized = normalizeLocaleTag(locale);
  if (new Intl.Locale(normalized).language.toLowerCase() === 'en') return {};

  const manifest = await loadManifest();
  if (!manifest) return null;

  for (const candidate of bundleCandidates(normalized)) {
    ...
    try {
      ...
      if (Object.keys(bundle).length !== artifact.sourceMessageCount) continue;
      bundleCache.set(candidate, bundle);
      return bundle;
    } catch {
      // Keep the current complete language active; never render a mixed-language page.
    }
  }
  return null;
}
...
export function I18nProvider({ children }: { children: ReactNode }) {
  ...
  const activate = useCallback((requestedLocale: string, persist = true) => {
    let normalized = 'en';
    try { normalized = normalizeLocaleTag(requestedLocale); } catch { normalized = 'en'; }
    const language = new Intl.Locale(normalized).language.toLowerCase();
    const target = hasCompleteTranslation(language) ? language : 'en';
    ...
    void loadCompleteBundle(target).then((bundle) => {
      if (requestId.current !== id) return;
      if (!bundle) {
        setTranslationStatus('error');
        return;
      }
      // Atomic switch: messages and document locale change together only after the full bundle exists.
      setMessages(bundle);
  ...
  const plural = useCallback((baseKey: string, count: number, params: MessageParams = {}, fallback?: string) => {
    ...
    const template = messages[categoryKey] ?? messages[otherKey] ?? sourceMessage(categoryKey, sourceMessage(otherKey, fallback));
  ...
  const value = useMemo<I18nContextValue>(() => ({
    ...
    t,
    plural,
    hasTranslation: hasCompleteTranslation,
