...
function fail(): never {
  throw new Error('Unsupported Sreva diagnostic technical state.');
}
...
export function buildDiagnosticReport(input: DiagnosticTechnicalInput): DiagnosticReport {
  ...
  if (!isTechnicalString(candidate?.appVersion, 64)) fail();
  if (!isTechnicalString(candidate?.browser?.name, 80)) fail();
  if (!isTechnicalString(candidate?.browser?.version, 80)) fail();
  ...
  if (!CRYPTO_ENGINES.has(candidate?.engine?.crypto)) fail();
  if (!PERSISTENCE_ENGINES.has(candidate?.engine?.persistence)) fail();
  if (!NOTIFICATION_PERMISSIONS.has(candidate?.permissions?.notifications)) fail();
  if (!STORAGE_PERMISSIONS.has(candidate?.permissions?.persistentStorage)) fail();
  ...
  return {
    ...
    browser: {
      name: candidate.browser.name,
      version: candidate.browser.version,
    },
    schema: {
      vaultVersion: candidate.schema.vaultVersion,
      supported: candidate.schema.supported,
    },
    engine: {
      crypto: candidate.engine.crypto,
      persistence: candidate.engine.persistence,
    },
    permissions: {
      notifications: candidate.permissions.notifications,
      persistentStorage: candidate.permissions.persistentStorage,
    },
    integrity: candidate.integrity.code
      ? { state: candidate.integrity.state, code: candidate.integrity.code }
      : { state: candidate.integrity.state },
  };
}
