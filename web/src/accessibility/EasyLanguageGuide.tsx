...
const SIMPLE_GUIDANCE: Record<string, string> = {
  ...
  reminders: 'Choose when Sreva should remind you and how much the notification should say.',
  ...
  insights: 'See patterns found in your own records. Sreva does not invent a diagnosis.',
  reports: 'Choose what to include, preview it, then export only if you want.',
  assistant: 'Type a simple command. Sreva explains it before changing private data.',
  sharing: 'Choose exactly what to share and review it before handoff.',
  vault: 'Create or restore an encrypted backup of your local Sreva data.',
  sync: 'Review optional encrypted continuity. Local Sreva still works without it.',
  ...
  more: 'Open any Sreva feature from one complete list.',
};

export function EasyLanguageGuide() {
  const pathname = usePathname();
  const section = pathname.split('/').filter(Boolean).at(-1) ?? 'home';
  const guidance = SIMPLE_GUIDANCE[section] ?? 'Use the controls on this page. Sreva keeps health actions explicit and private.';
  return (
